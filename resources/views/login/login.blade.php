                        ...
                        <div class="card">
                            <div class="card-body p-4">
                                <div class="text-center mt-2">
                                    ...
                                    <p class="text-muted">{{ $language['LoginSignTitle'] }} {{ $language['NomeEmpresa'] }}.
                                    </p>
                                </div>
                                <div class="p-2 mt-4">
                                    ...
                                    <form action="{{ route('login.auth') }}" method="POST">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" name="email" id="email"
                                                required placeholder="{{ $language['EmailUser'] }}">
                                        </div>

                                        <div class="mb-3">
                                            <div class="float-end">
                                                <a href="/login/recoverpw"
                                                    class="text-muted">{{ $language['LoginForgot'] }}</a>
                                            </div>
                                            <label class="form-label"
                                                for="password">{{ $language['LoginPassword'] }}</label>
                                            <input type="password" class="form-control" required name="password"
                                                id="password" placeholder="{{ $language['LoginPassword'] }}">
                                        </div>

                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="remember" name="remember">
                                            <label class="form-check-label"
                                                for="remember">{{ $language['LoginRemember'] }}</label>
                                        </div>

                                        <div class="mt-3 text-end">
                                            <button class="btn btn-primary w-sm waves-effect waves-light"
                                                type="submit">{{ $language['LoginLog'] }}</button>
                                        </div>
                                        ...
                                        <div class="mt-4 text-center">
                                            <p class="mb-0">{{ $language['LoginDont'] }} <a href="/login/register"
                                                    class="fw-medium text-primary"> {{ $language['LoginSign'] }} </a> </p>
                                        </div>
                                    </form>
                                </div>
